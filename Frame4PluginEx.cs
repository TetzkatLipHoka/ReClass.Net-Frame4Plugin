...
//                host.Logger.Log(ex);
            ...
            if (DefaultPlugin == 1) {
                host.Process.CoreFunctions.SetActiveFunctionsProvider("Frame4 Loader");
            }
...
*/
        ...
        SectionProtection GetProtectionLevel(uint prot)
        {
            ...
            if ((prot & (uint)FRAME4.VM_PROTECTIONS.VM_PROT_READ) == (uint)FRAME4.VM_PROTECTIONS.VM_PROT_READ)
            {
                sectionProtection |= SectionProtection.Read;
            }

            if ((prot & (uint)FRAME4.VM_PROTECTIONS.VM_PROT_WRITE) == (uint)FRAME4.VM_PROTECTIONS.VM_PROT_WRITE)
            {
                sectionProtection |= SectionProtection.Read | SectionProtection.Write;
            }

            if ((prot & (uint)FRAME4.VM_PROTECTIONS.VM_PROT_DEFAULT) == (uint)FRAME4.VM_PROTECTIONS.VM_PROT_DEFAULT)
            {
                sectionProtection |= SectionProtection.Read | SectionProtection.Write;
            }

            if ((prot & (uint)FRAME4.VM_PROTECTIONS.VM_PROT_EXECUTE) == (uint)FRAME4.VM_PROTECTIONS.VM_PROT_EXECUTE)
            {
                sectionProtection |= SectionProtection.Execute;
            }

            if ((prot & (uint)FRAME4.VM_PROTECTIONS.VM_PROT_READEXEC) == (uint)FRAME4.VM_PROTECTIONS.VM_PROT_READEXEC)
            {
                sectionProtection |= SectionProtection.Read | SectionProtection.Execute;
            }

            if ((prot & (uint)FRAME4.VM_PROTECTIONS.VM_PROT_ALL) == (uint)FRAME4.VM_PROTECTIONS.VM_PROT_ALL)
            {
